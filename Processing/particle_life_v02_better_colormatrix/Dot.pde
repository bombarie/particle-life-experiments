...
class Dot {
  ...
  public void update(float noiseIncr) {
    ...
    float noiseFinalStrength = 23.9;

    this.position.x += (noise(position.x / 100.0 + noiseIncr) - .49) * noiseStrength * noiseFinalStrength;
    this.position.y += (noise(position.y / 100.0 + noiseIncr) - .49) * noiseStrength * noiseFinalStrength;
